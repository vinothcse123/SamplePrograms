...
int OCIConnection::insertToTable()
{
    ...
    const int INT_BUFFER_1_SIZE = 3;
    const int INT_BUFFER_2_SIZE = 2;
    ...
    int intBuffer_1[INT_BUFFER_1_SIZE] = {};
    int intBuffer_2[INT_BUFFER_2_SIZE] = {};
    double doubleBuffer[ROW_COUNT] = {};
    OCIIOV  ioVec[NUMBER_OF_BUFFERS];
    ...
    for(int i=0;i<INT_BUFFER_1_SIZE;i++)
    {
        intBuffer_1[i]=100+i;
    }

    for(int i=0;i<INT_BUFFER_2_SIZE;i++)
    {
        intBuffer_2[i]=200+i;
    }
        
    //static text *insertstr = (text *)"INSERT INTO V6Play1 (MyNumber,MyNumber2) VALUES (:1,:2,:3)";
    ...
    ioVec[0].bfp=intBuffer_1;
    ioVec[0].bfl=INT_BUFFER_1_SIZE*sizeof(intBuffer_1[0]);
    ioVec[1].bfp=intBuffer_2;
    ioVec[1].bfl=INT_BUFFER_2_SIZE*sizeof(intBuffer_2[0]);
    IS_ERROR(pOciErrorHandle, OCIBindByPos(stmthp, &ociBind, pOciErrorHandle, 1,(void *) &ioVec,
